...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        var _config = that.config;
        var tabIndex = that.exists(data.menu_type);
        if (tabIndex === -1) {
            //设置只能同时打开多少个tab选项卡
            ...
            globalTabIdIndex++;
            var content = '<iframe src="' + data.href + '" id="iframe_'+globalTabIdIndex+'" data-id="' + globalTabIdIndex + '" data-menu-type="' + data.menu_type + '"></iframe>';
            var title = '';
            ...
            title += '<cite parent-menu-type="'+data.parent_menu_type+'">' + data.menu_type + '</cite><font>' + data.title + '</font>';
            if (_config.closed) {
            ...
            //切换到当前打开的选项卡
            element.tabChange(ELEM.tabFilter, that.getTabId(data.menu_type));
        } else {
            element.tabChange(ELEM.tabFilter, that.getTabId(data.menu_type));
            //自动刷新
	 ...
	 * @param {String} id
	 */
    Tab.prototype.changeTab = function (menu_type) {
        var that = this;
        element.tabChange(ELEM.tabFilter, that.getTabId(menu_type));
        return that;
    }
